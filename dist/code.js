...
var devDependencies = {
	"svelte-components": "https://github.com/fignite/svelte-components/tarball/main",
...
const nodeToObject = (node, options) => {
    ...
    if (node.parent && !(options === null || options === void 0 ? void 0 : options.withoutRelations)) {
        obj.parent = { id: node.parent.id, type: node.parent.type };
    }
    if (node.children && !(options === null || options === void 0 ? void 0 : options.withoutRelations)) {
        obj.children = node.children.map((child) => nodeToObject(child, options));
    }
    if (node.masterComponent && !(options === null || options === void 0 ? void 0 : options.withoutRelations)) {
    ...
    // PluginData
    if ((options === null || options === void 0 ? void 0 : options.pluginData) && node.getPluginDataKeys().length > 0) {
        obj.pluginData = {};
    ...
    if (options === null || options === void 0 ? void 0 : options.sharedPluginDataNamespaces) {
        ...
        options === null || options === void 0 ? void 0 : options.sharedPluginDataNamespaces.forEach((namespace) => {
            ...
            node.getSharedPluginDataKeys(namespace).forEach((key) => {
                obj.sharedPluginData[namespace][key] = node.getSharedPluginData(namespace, key);
            });
...
function move$1(array, from, to, replaceWith) {
    // Remove from array
...
async function addRecentFileAsync(file) {
    return await updateClientStorageAsync("recentFiles", (recentFiles) => {
        recentFiles = recentFiles || [];
        recentFiles = upsert$1(recentFiles, (item) => item.id === file.id, file);
        return recentFiles;
    });
}
...
var addRecentFileAsync_1 = addRecentFileAsync;
...
function groupBy$1(list, keyGetter) {
    const map = new Map();
    list.forEach((item) => {
...
let selectTableCellsRelaunchData = {
...
async function copyTemplatePart(partParent, node, index, templateSettings, tableSettings, rowIndex, opts) {
    ...
    if (tableSettings) {
        if (templateCell) {
            // Copy across width
            if (!(opts === null || opts === void 0 ? void 0 : opts.ignore.includes('width'))) {
...
async function createTable(templateComponent, settings, type, templateData) {
    ...
    if (!part.table || !part.tr || !part.td || (!part.th && settings.header)) {
        ...
        if (tableIsContainer) {
            ...
            nodeToReplace.parent.insertChild(getNodeIndex_1(nodeToReplace), table);
...
function getLocalTemplates() {
    figma.skipInvisibleInstanceChildren = true;
    var fileId = getDocumentData_1('fileId');
    var templates = [];
    var components = figma.root.findAllWithCriteria({
        types: ['COMPONENT'],
    });
    for (let i = 0; i < components.length; i++) {
        ...
        if (templateData && node.type === 'COMPONENT') {
            ...
            if (templateData.id !== node.id && templateData.file.id !== fileId) {
                setPluginData_1(node, 'prevTemplate', templateData);
            ...
            if (componentTimestamp < currentTimestamp - daysToMilliseconds(35)) {
                setPluginData_1(node, 'prevTemplate', '');
            }
        }
    }
...
async function getDefaultTemplate() {
    ...
    let defaultTemplate;
    if ((defaultTemplates === null || defaultTemplates === void 0 ? void 0 : defaultTemplates.length) > 0) {
    ...
    if (defaultTemplate && !isEmpty$1(defaultTemplate)) {
        if (defaultTemplate.file.id === fileId) {
            ...
            if (!templateComponent) {
                ...
                if ((remoteFiles === null || remoteFiles === void 0 ? void 0 : remoteFiles.length) > 0) {
                    defaultTemplate = remoteFiles[0].data[0];
                }
                else if ((localTemplates === null || localTemplates === void 0 ? void 0 : localTemplates.length) > 0) {
                    defaultTemplate = localTemplates[0];
                }
                else {
...
async function updateTables(template) {
    ...
    for (let p = 0; p < pages.length; p++) {
        ...
        var tableInstances = page.findAll((node) => {
            var _a, _b;
            if (((_b = (_a = getPluginData(node, 'template')) === null || _a === void 0 ? void 0 : _a.component) === null || _b === void 0 ? void 0 : _b.id) === template.component.id && node.type === 'FRAME') {
                return true;
            }
        });
...
function alignCell(selection, alignment) {
        ...
        if (oldTable.type !== 'COMPONENT') {
            ...
            if (firstTableHasColumnResizing) {
                ...
                for (let a = 0; a < rowLength; a++) {
                    ...
                    if (((_a = getPluginData_1(oldTableRow, 'elementSemantics')) === null || _a === void 0 ? void 0 : _a.is) === 'tr') {
                        ...
                        for (let b = 0; b < columnLength; b++) {
                            ...
                            if (((_b = getPluginData_1(oldTableCell, 'elementSemantics')) === null || _b === void 0 ? void 0 : _b.is) === 'td' ||
                                ((_c = getPluginData_1(oldTableCell, 'elementSemantics')) === null || _c === void 0 ? void 0 : _c.is) === 'th') {
                ...
                if (getPluginData_1(oldTable.parent, 'template')) {
...
async function createTableUI() {
    ...
    var recentFiles = await getRecentFilesAsync_1(localTemplates, { limit: 8 });
                            ...
                            for (let i = 0; i < convertToNumber(parameters.number); i++) {
                                {
                                    ...
                                    let componentSet = (_a = cellTemplate === null || cellTemplate === void 0 ? void 0 : cellTemplate.mainComponent) === null || _a === void 0 ? void 0 : _a.parent;
                                    if (!componentSet) {
                                    ...
                                    if (isHeaderCell(cell) && !cellsDetached) {
