...
async function updateTables(template) {
    ...
    for (let p = 0; p < pages.length; p++) {
        ...
        await sleep$1();
        var tableInstances = page.findAll((node) => { var _a; return ((_a = getPluginData_1(node, 'template')) === null || _a === void 0 ? void 0 : _a.id) === template.id && node.type === 'FRAME'; });
        if (templateComponent && tableInstances) {
            ...
            for (let b = 0; b < tableInstances.length; b++) {
                let tableParts = getTemplateParts$1(tableInstances[b]);
