<script>
	...
	// 	// Reset to no state
	// 	set_state('selected', [grid[0], grid[1]], false);
	// 	set_state('selected', [value.columns - 1, value.rows - 1]);
	// });
	
	function on_load(node) {

		// Set default checked radio
		table_state[columns - 1][rows - 1].checked = true;
		
		// Set default state
		set_state('selected', [columns - 1, rows - 1]);
	}
...
<table>
	...
	{#each {length: grid[0]} as _, x}
	  ...
	  {#each {length: grid[1]} as _, y}
			<td class:hover={table_state[x][y].hover}
