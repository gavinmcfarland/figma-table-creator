	...
	function updateSelectedFile(data, file) {
		...
		if (!data && !file) {
			...
			if (file) {
				...
				let index = data.localTemplates.findIndex(
					(template) =>
						template.component.id === data?.defaultTemplate?.component.id &&
						template.component.key === data?.defaultTemplate?.component.key
				)
...
{#if pageState.chooseRemoteTemplate}
	<div class="container choose-library" style="padding: var(--size-100) var(--size-200)">
		{#if showToggles}
			<div class="TopBar">
				<span style="display: flex; align-items: center;">
					<a title="Back" class="refresh icon" icon="chevron-left" on:click={() => setActivePage('createTablePageActive')} />
		...
		{:else}
			<div class="TopBar">
				<span style="display: flex; align-items: center;">
					<a title="Back" class="refresh icon" icon="chevron-left" on:click={() => setActivePage('welcomePageActive', 4)} />
					<span style="font-weight: bold; margin-left: 4px; margin-top: 2px">Libraries</span>
				</span>
			</div>
		{/if}
		...
		<div class="main-content">
					...
					{#each data.recentFiles as file}
						{#if showToggles}
							<div
								...
								<span>{file.name}</span><span style="margin-right: -8px" class="icon" icon="chevron-right" />
							</div>
						{:else}
							<div
								class="ListItem"
			...
			{/if}
			<p class="description">
				If you can't see your library listed, open the Assets panel and drag an instance of each template onto the canvas.
			</p>
			<!-- <div class="help">
			<div class="artwork2">
				<div class="svg8"></div>
...
{#if pageState.chooseTemplate}
	<div class="container" style="padding: var(--size-100) var(--size-200)">
		<div class="TopBar">
			<span style="display: flex; align-items: center;">
				<a title="Back" class="refresh icon" icon="chevron-left" on:click={() => setActivePage('chooseRemoteTemplate')} />
...
{#if pageState.welcomePageActive}
	{#if welcomeSlides[0]}
		...
		<div class="container welcomePage" style="padding: var(--size-200)">
			...
			<div class="content">
				<h6>What's new</h6>
				<p>Table Creator has been rebuilt from the ground up with some new features.</p>
				<div class="buttons">
	...
	{#if welcomeSlides[1]}
		<div class="container welcomePage" style="padding: var(--size-200)">
			...
			<div class="content">
				<h6>Templates</h6>
				<p>
					Tables are now created from a single component called a template. Easily update already created tables directly from the template.
				</p>
				<div class="buttons">
					<span class="prev" on:click={() => setActiveSlide(0)}><Button classes="tertiary" iconRight="arrow-left" /></span>
	...
	{#if welcomeSlides[2]}
		<div class="container welcomePage" style="padding: var(--size-200)">
			...
			<div class="content">
				...
				<p>
					Manage more than one table design by creating multiple templates. Choose the template you want by selecting it from the dropdown
					when creating a table.<br />
				</p>
				<div class="buttons">
					<span class="prev" on:click={() => setActiveSlide(1)}><Button classes="tertiary" iconRight="arrow-left" /></span>
	...
	{#if welcomeSlides[3]}
		<div class="container welcomePage" style="padding: var(--size-200)">
			...
			To use templates with libraries, first publish the template and then run the plugin in another file.
				...
				<p>Templates can be used with libraries. Simply publish the template and then import it from any of your files.</p>
				<div class="buttons">
					<span class="prev" on:click={() => setActiveSlide(2)}><Button classes="tertiary" iconRight="arrow-left" /></span>
					<span
						class="next"
						on:click={() => {
		...
		<div class="container welcomePage" style="padding: var(--size-200)">
			{#if data.pluginUsingOldComponents}
				...
				<div class="content">
					<h6>Update tables</h6>
					<p>
						The tables in this file need updating. This will update your existing tables and convert your table components into a single
			...
			{/if} -->
					...
					<p>Create a new template or import an existing template from a library.</p>
